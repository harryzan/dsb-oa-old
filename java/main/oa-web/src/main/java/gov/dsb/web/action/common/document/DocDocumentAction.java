...
@Results({@Result(name = CRUDActionSupport.RELOAD, location = "doc-document-view", type = "chain")})
public class DocDocumentAction extends CRUDActionSupport<DocDocument> {
    ...
    public String save() throws Exception {
        ...
        if (attachids != null && attachids.length > 0) {
            for (int i = 0; i < attachids.length; i++) {
                ...
                DocDocumentAttach attach = docDocumentAttachEntityService.get(attachid);
                ...
                docDocumentAttachEntityService.save(attach);
                map.remove(attachid);//从map中删除已经更新的附件
            }
        }
        for (DocDocumentAttach d : map.values()) {
            docDocumentAttachEntityService.delete(d.getId()); //map中剩余的都是要要删除的附件
        }
        ...
        if (doccategoryid != null) {
            ...
            if (formatid != null) {
                SysCodeList format = sysCodeListEntityService.get(formatid);
                entity.setFormat(format);
            }
            if (propertyid != null) {
                SysCodeList property = sysCodeListEntityService.get(propertyid);
                entity.setProperty(property);
            }
            if (deptid != null) {
                SysDept dept = sysDeptEntityService.get(deptid);
            ...
            if (docattachs != null && docattachs.size() > 0) {
                for (DocDocumentAttach docattach : docattachs) {
                    if (docattach != null) {
                        ...
                        docDocumentAttachEntityService.save(docattach);
                    }
                }
            }
        }
    ...
    private Collection<DocDocumentAttach> fileOperation() throws IOException {
        ...
        for (int i = 0; i < upload.length; i++) {
            try {
                ...
                docDocumentAttachEntityService.save(temp);
                result.add(temp);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    protected void prepareModel() throws Exception {
        ...
        formats = sysCodeEntityService.findCodeList("format");
        properties = sysCodeEntityService.findCodeList("property");
    }
