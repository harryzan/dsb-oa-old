...
public class ItemGridAction extends PageActionSupport<Item> {
    ...
    public String griddata() throws Exception {
        if (!Nulls.isNull(limit) && limit > 0) {
            page = new Page<Item>(limit, true);
        }
        else {
            page = new Page<Item>(10, true);
        }
        ...
        if (!StringHelp.isEmpty(conditions)) {
            QueryTranslate queryTranslate = new QueryTranslate("from Item", conditions);
            page = service.findPageByQuery(page, queryTranslate.toString());
        }
        else {
            page = service.findAll(page);
        }
        List<Item> list = page.getResult();
