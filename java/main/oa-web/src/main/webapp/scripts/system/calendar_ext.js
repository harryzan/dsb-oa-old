...
strFrame += '         onmouseover="style.color=\'#8E8E8E\'" onmouseout="style.color=\'#1B386D\'" title="当前日期时间">当前</span>&nbsp;&nbsp;<span style="cursor:pointer" id=evaAllOK onmouseover="style.color=\'#8E8E8E\'" onmouseout="style.color=\'#1B386D\'"  onclick="parent.closeLayer()" title="关闭日历">关闭&nbsp;</span></td></tr>';
...
function setday(tt, obj, useTime) //主调函数
{
    ...
    //add by lerry
    //当图标下方的空间不足时改变弹出方向为向上
    if ((document.body.clientHeight - event.clientY) < 218)
    {
        dads.top = ((ttyp == "image") ? ttop + thei - 232 : ttop + thei + 6 - 232);
    }
    if ((document.body.clientWidth - event.clientX) < 170)
    {
        dads.left = ((ttyp == "image") ? tleft - 170 : tleft + 6 - 170);
    }
    //add by lerry
    ...
    var r = outObject.getValue().match(reg);
    if (r != null)
    {
        r[2] = r[2] - 1;
...
function meizzToday()    //Today Button
{
    ...
    if (outObject)
    {
        if (bUseTime)
        {
            outObject.setValue(parent.meizzTheYear + "-" + format(parent.meizzTheMonth) + "-" +
...
function WriteDateTo(yy, mm, n, hh, mi, se)
{
    if (bUseTime)
    {
        outObject.setValue(yy + "-" + format(mm) + "-" + format(n) + " " + format(hh) + ":" + format(mi) + ":" + format(se)); //注：在这里你可以输出改成你想要的格式
    }
    else
    {
        outObject.setValue(yy + "-" + format(mm) + "-" + format(n)); //注：在这里你可以输出改成你想要的格式
    }
}
